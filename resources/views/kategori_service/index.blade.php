...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center py-3 mb-4">
            ...
            <a href="{{ route('kategori_service.create') }}" class="btn btn-primary shadow-sm">
                <i class="bx bx-plus me-1"></i> Tambah Kategori Service
            </a>
        </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header border-bottom bg-transparent py-3">
                <h5 class="card-title mb-0">Daftar Kategori Service</h5>
            </div>
            ...
            <div class="table-responsive text-nowrap">
                <table class="table table-hover align-middle mb-0" id="example3">
                    ...
                    <tbody>
                        @foreach ($kategori_service as $row)
                            <tr>
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-xs me-2">
                                            <span class="avatar-initial rounded bg-label-primary text-uppercase"
                                                ...
                                                {{ substr($row->nama_kategori_service, 0, 1) }}
                                            </span>
                                        </div>
                                        <span class="fw-semibold text-dark">{{ $row->nama_kategori_service }}</span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('kategori_service.edit', $row->id_kategori_service) }}"
                                            class="btn btn-icon btn-sm btn-outline-primary" data-bs-toggle="tooltip"
                                            title="Edit Data">
                                            <i class="bx bx-edit-alt"></i>
                                        </a>

                                        <form action="{{ route('kategori_service.destroy', $row->id_kategori_service) }}"
        ...
        document.querySelectorAll('.show_confirm').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Data kategori_service ini akan dihapus secara permanen!",
                    icon: 'warning',
