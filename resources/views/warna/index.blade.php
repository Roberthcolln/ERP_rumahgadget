...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center py-3 mb-4">
            ...
            <a href="{{ route('warna.create') }}" class="btn btn-primary shadow-sm">
                <i class="bx bx-plus me-1"></i> Tambah warna
            </a>
        </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header border-bottom bg-transparent py-3">
                <h5 class="card-title mb-0">Daftar warna Produk</h5>
            </div>
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-xs me-2">
                                            <span class="avatar-initial rounded bg-label-primary text-uppercase"
                                                ...
                                                {{ substr($row->nama_warna, 0, 1) }}
                                            </span>
                                        </div>
                                        <span class="fw-semibold text-dark">{{ $row->nama_warna }}</span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('warna.edit', $row->id_warna) }}"
                                            class="btn btn-icon btn-sm btn-outline-primary" data-bs-toggle="tooltip"
                                            title="Edit Data">
                                            <i class="bx bx-edit-alt"></i>
                                        </a>

                                        <form action="{{ route('warna.destroy', $row->id_warna) }}" method="POST"
                                            class="m-0">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
        ...
        document.querySelectorAll('.show_confirm').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Data warna ini akan dihapus secara permanen!",
                    icon: 'warning',
