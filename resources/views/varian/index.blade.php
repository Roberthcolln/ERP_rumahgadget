...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center py-3 mb-4">
            ...
            <a href="{{ route('varian.create') }}" class="btn btn-primary shadow-sm">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header border-bottom bg-transparent py-3">
                <h5 class="card-title mb-0">Daftar Varian Produk</h5>
            </div>
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-xs me-2">
                                            <span class="avatar-initial rounded bg-label-primary text-uppercase"
                                                ...
                                                {{ substr($row->nama_varian, 0, 1) }}
                                            </span>
                                        </div>
                                        <span class="fw-semibold text-dark">{{ $row->nama_varian }}</span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('varian.edit', $row->id_varian) }}"
                                            class="btn btn-icon btn-sm btn-outline-primary" data-bs-toggle="tooltip"
                                            title="Edit Data">
                                            <i class="bx bx-edit-alt"></i>
                                        </a>

                                        <form action="{{ route('varian.destroy', $row->id_varian) }}" method="POST"
                                            class="m-0">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
        ...
        document.querySelectorAll('.show_confirm').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Data varian ini akan dihapus secara permanen!",
                    icon: 'warning',
